...
public class TextureEmpty3D implements Texture {
    ...
    public TextureEmpty3D(int size) {
        this.sizes = new Vector3i[size];
     ...
     */
    public void initTexture(int index, int unit, Vector3i size, ImageType type) {
        glActiveTexture(GL_TEXTURE0 + unit);
        glBindTexture(GL_TEXTURE_3D, textureBuffers[index]);

        this.sizes[index] = size;
        this.levels[index] = unit;
        this.online[index] = true;
        this.types[index] = type;
        glTexImage3D(GL_TEXTURE_3D, 0, type.toInternalFormat(), size.getX(), size.getY(), size.getZ(), 0, type.toFormat(),
     ...
     */
    public void initTextures(@NotNull int[] units, @NotNull Vector3i[] sizes, @NotNull ImageType[] types) {
