...
public class GLSLDynamicStructArray extends DynamicStructArray<Struct> {

    private long byteSize;
    private int padding;

    public GLSLDynamicStructArray() {}

    public GLSLDynamicStructArray(Struct[] data) {
        super(data);
    }
    ...
    public void bake(MemoryLayout layout, long off) throws IllegalStructSizeException {
        ...
        for (int i = 0; i < size(); i++) {
            Struct element = get(i);
            element.bake(layout, offset);

            if (i == 0) {
                structSize = element.byteSize();
                ...
                if (element.byteSize() != structSize)
                    throw new IllegalStructSizeException(element.byteSize(), structSize);
            }
